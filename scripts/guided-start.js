...
function printClearSessionHint(config) {
  console.log('');
  console.log('登入狀態會保存在這個工作瀏覽器資料夾中：');
  console.log(config.userDataDir);
  console.log('');
  console.log('如果之後想清除這個登入狀態，可以告訴 AI 助手：「請清除這個工作瀏覽器的登入狀態」。');
  console.log('AI 助手會在關閉工作瀏覽器後協助刪除：');
  console.log(`- ${config.userDataDir}`);
...
async function createNewSession(rl) {
  ...
  console.log('登入狀態會自動保存在這個專案底下，之後可以重用，不用每次重新登入。');
  console.log('如果之後想清除，可以直接告訴 AI 助手要清除登入狀態。');
...
async function reuseSavedSession(rl) {
  ...
  try {
    ...
    console.log('');
    console.log('我找到之前的工作瀏覽器設定，但目前沒有連上。');
    console.log('我會用原本的登入資料夾重新開啟它，請稍等。');

    try {
      const portIsFree = await isPortFree(selected.session.port);
      if (!portIsFree) {
        throw new Error(`Port ${selected.session.port} 目前被其他程式使用。`);
      }

      const launchedPid = launchBrowser(selected.session);
      await waitForCdpEndpoint(selected.session.cdpUrl);
      const verification = await verifyPlaywrightConnection(selected.session.cdpUrl);
      const updatedConfig = {
        ...selected.session,
        launchedPid,
      ...
      console.log('');
      console.log('這個工作瀏覽器無法自動重新開啟。');
      console.log(reopenError.message || reopenError);
